...
export default function OnboardingFeedbackSurveyPage() {
  ...
  const allRated = answers.q1 > 0 && answers.q2 > 0 && answers.q3 > 0 && answers.q4 > 0;

  const handleSubmit = async () => {
    if (!allRated) {
      toast.error("Please rate all four questions before submitting.");
      return;
    }
    setSaving(true);
    try {
        ...
        >
          {submitted ? (
            ...
            <>
              ...
              <QuestionCard
                index={4}
                label="How welcomed and supported did you feel during your first 30 days?"
                hint="1 – Not welcomed · 5 – Extremely welcomed"
              >
                <EmojiRating value={answers.q4} onChange={(v) => set("q4", v)} />
              </QuestionCard>

              {/* Q5 */}
              <QuestionCard
                index={5}
                label="What aspects of the onboarding experience stood out positively?"
              >
                <textarea
                  value={answers.q5}
              ...
              <QuestionCard
                index={6}
                label="What improvements would you recommend for enhancing the onboarding experience?"
              >
                <textarea
                  value={answers.q6}
              ...
              <QuestionCard
                index={7}
                label="How well did you integrate into the leadership ecosystem?"
              >
                <textarea
                  value={answers.q7}
