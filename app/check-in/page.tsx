...
export default function CheckIn() {
  ...
  const handleSubmit = async () => {
    if (!allRated) {
      toast.error("Please rate all required questions before submitting.");
      return;
    }
    setSaving(true);
    try {
        ...
        >
          {submitted ? (
            ...
            <>
              ...
              {/* Q4 — readiness block (objective) */}
              <QuestionCard
                index={4}
                label="Please rate the readiness of the following on your joining:"
              >
                <div className="mt-3 space-y-4">
                  <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-4">
                    <p className="text-sm font-semibold text-slate-800">Laptop / Devices</p>
                    <EmojiRating value={answers.q5} onChange={(v) => set("q5", v)} />
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-4">
                    <p className="text-sm font-semibold text-slate-800">Onboarding Kit</p>
                    <EmojiRating value={answers.q6} onChange={(v) => set("q6", v)} />
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-4">
                    <p className="text-sm font-semibold text-slate-800">Email and Collaboration tools</p>
                    <EmojiRating value={answers.q7} onChange={(v) => set("q7", v)} />
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-4">
                    <p className="text-sm font-semibold text-slate-800">Access to internal tools (HRMS/Enable etc)</p>
                    <EmojiRating value={answers.q8} onChange={(v) => set("q8", v)} />
                  </div>
                </div>
              </QuestionCard>

              {/* Q5 — subjective */}
              <QuestionCard index={5} label="What has been the most positive part about your journey?">
                <textarea
                  value={answers.q4}
              ...
              <QuestionCard
                index={6}
                label="Were your role expectations and success metrics clearly defined?"
              >
                <textarea
                  value={answers.q9}
