...
const VIDEO_SRC = "/videos/module-3.mp4";
...
export default function HRInductionModule3() {
  ...
  const onEnded = () => {
    ...
    fetch("/api/mark-complete", {
      ...
      body: JSON.stringify({ module: "hr-induction-module-3" }),
    }).catch(() => {});
  };
          ...
          <div className="border-t border-white/5 bg-slate-900/60 px-4 py-4 sm:px-6">
            ...
            {ended && (
              <p className="mt-4 rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-sm text-emerald-200">
                You&apos;ve finished Module 3. Great job - head back to pick your next module.
              </p>
            )}
          </div>
        </div>
